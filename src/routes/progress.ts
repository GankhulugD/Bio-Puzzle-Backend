...
function quizKeyForGame(gameKey: string) {
  return `game:${gameKey}`;
}

/** Бүлгийн асуултаас буцаж ирэхэд — бүх хичээлийг дуусгасан гэж үзнэ (quiz бүх асуултыг агуулдаг). */
router.post("/chapter-quiz", requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const { chapterId, correctCount, totalCount } = req.body as {
      chapterId?: string;
    ...
    const lessonRows = await prisma.curriculumLesson.findMany({
      where: { chapterId },
      select: { id: true },
    });
    ...
    const user = await applyPointsAndStreak(userId, pointsDelta);
    res.status(201).json({
      user,
...
router.post("/mini-game", requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const gameKey =
      typeof body.gameKey === "string" ? body.gameKey.trim() : "";
