...
import type { CurriculumLesson } from "@prisma/client";
import { prisma } from "../lib/prisma";

const router = Router();

function mapLesson(row: CurriculumLesson) {
  return {
    id: row.id,
    type: row.type,
    title: row.title,
...
router.get("/bootstrap", async (_req, res) => {
  try {
    ...
    const lessonsByChapter: Record<string, ReturnType<typeof mapLesson>[]> =
      {};
    ...
    const pick = (key: string) =>
      blobs.find((b) => b.key === key)?.data ?? null;
...
router.get("/chapters/:chapterId/lessons", async (req, res) => {
  try {
    ...
    const rows = await prisma.curriculumLesson.findMany({
      where: { chapterId },
      orderBy: { sortOrder: "asc" },
    });
